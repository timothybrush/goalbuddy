...
- **Mixed fleets: one board, many vendors.** A PM in any harness can dispatch a single task to a different vendor's agent — an Opus judge with a Codex worker and a Gemini scout on the same board. `goalbuddy dispatch docs/goals/<slug> --to codex` (also `claude-code`, `gemini`; bundled as a skill script for model-invoked use) renders the task prompt, runs the target CLI headless with role-appropriate sandboxing, extracts the returned receipt, and mechanically verifies write scope with git — worker changes must match `allowed_files`, read-only roles must change nothing. The dispatcher never touches `state.yaml`; the PM records the receipt, stamped with the harness that earned it.
